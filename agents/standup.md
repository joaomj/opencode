...
2. If that fails, return error to main agent
3. Main agent can manually retry with different model

### Git Fallback
If GitHub CLI (`gh`) fails:
1. Fall back to local git commands only
2. Inform user that PR/Issue data unavailable
3. Continue with commit-based analysis only
