...
Write a self-contained HTML file to the OS temp directory so nothing lands in the repo. Resolve the temp dir from the environment (`$TMPDIR` on macOS/Linux, `/tmp` as universal fallback), and write to `<tmpdir>/architecture-review-<timestamp>.html` so each run gets a fresh file. Open it for the user with the platform's default browser opener and tell them the absolute path.
...
Once the user picks a candidate, use a grilling interview pattern — the same relentless, one-question-at-a-time approach from `/grill-with-docs` — to walk the decision tree with them: constraints, dependencies, the shape of the deepened module, what sits behind the seam, what tests survive.
