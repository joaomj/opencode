...
3. Write the spec using the template below, then publish it to the project issue tracker. Apply the `ready-for-agent` label — no need for additional triage.
...
`to-spec` actively looks for deep module opportunities — modules that hide meaningful complexity behind a small, stable, testable interface.
...
Use after domain language and plan are resolved, before breaking into tickets.
